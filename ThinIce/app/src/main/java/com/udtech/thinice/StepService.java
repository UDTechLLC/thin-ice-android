...
//                }
    ...
    public void onEvent(PairDevice event) {
        ...
        BluetoothDevice bDevice = event.getBluetoothDevice();
        if (pairDevice(bDevice)) {
            new ConnectionThread(bDevice, device).start();
        } else {
            unpairDevice(bDevice);
            pairDevice(bDevice);
            new ConnectionThread(bDevice, device).start();
        }
    }
